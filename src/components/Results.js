...
import result from './Results/result1.xlsx'; // Ensure this path is correct

function Results() {
    const [data, setData] = useState([]); // State to hold the data from the Excel file
    const [loading, setLoading] = useState(true); // State to manage loading status
            ...
            fileReader.onload = (e) => {
                ...
                const formattedData = rows.map((row, index) => {
                    return headers.reduce((acc, header, i) => {
                        acc[header] = row[i]; // Create an object for each row
                        return acc;
    ...
    const handleDownload = () => {
        ...
        XLSX.writeFile(workbook, './Results/results.xlsx'); // Download the file
    };

    useEffect(() => {
        fetchExcelData(); // Fetch data when the component mounts
    }, []);
    ...
    const handleEmail = () => {
        ...
        // Fix: Write workbook as an array buffer
        ...
        // Open Outlook (User must attach the file manually)
        const subject = encodeURIComponent("Risk Report");
        const body = encodeURIComponent("Please find the attached risk report. Attach the 'results.xlsx' file from your Downloads folder.");
                ...
                <div className="results-table-container1">
                    <div className='results-table-container1-heading'>
                        ...
                        <div className="button-group">
                            ...
                            <button className="refresh-btn" onClick={fetchExcelData}>
                                Refresh Data
                            </button>
